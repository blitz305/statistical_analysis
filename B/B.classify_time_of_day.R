...
# 创建一个新的主表，并直接在上面操作
mvpa_time_pattern_new <- Derived_accelerometry %>%
  # 选择需要的列
  select(
...
# 3. 将Balanced/Tied转换为NA
# ==========================================
print("步骤 C: 将Balanced/Tied转换为NA...")
mvpa_time_pattern_new <- mvpa_time_pattern_new %>%
  mutate(
    activity_time_pattern = na_if(activity_time_pattern, "Balanced/Tied")
  )
print("步骤 C 完成。")

# 4. 汇总信息 (与您的版本类似)
# ==========================================
print("===== 分类结果分布（Balanced已转为NA）=====")
print(table(mvpa_time_pattern_new$activity_time_pattern, useNA = "ifany"))

# 5. 重命名变量并抽取主暴露表供后续分析使用
# ==========================================
mvpa_time_pattern <- mvpa_time_pattern_new

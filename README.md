...
This repository packages the end-to-end statistical workflows built on UK Biobank accelerometry and registry data. It covers cognitive feature processing (Phase A), activity regularity and exposure clustering (Phase B), plus survival/clinical outcome modeling (Phase C). Alongside the main pipelines it ships a reproducible environment, quick validation scripts, and code for manuscript tables.

## Repository Layout
- `cognitive_features/` (formerly `A/`): cleans and summarizes raw cognitive tests and screening fields. Use `A.cognitive_result.R` as the entry point.
...
- `table1/`: scripts that generate published tables.
...
3. **R version**: use the version recorded by `renv` (typically 4.x) to avoid untested runtime differences.
...
Keep README updates in this structure and tone when documenting new directories or phases so collaborators can onboard quickly.

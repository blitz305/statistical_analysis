...
# 作者: AI Assistant
# 日期: 2024
# 描述: 绘制认知变化与睡眠总时长（控制所有协变量）的快速平滑图，用于决定模型选择
...
plot_data_full <- full_merged_table %>%
  ...
  filter(!is.na(cognitive_change) & !is.na(sleep_hours_per_day))

print(paste("剔除缺失值后样本数:", nrow(plot_data_full)))

# 剔除睡眠时间异常值（限定于6-10小时）
print("正在剔除睡眠时间异常值（限定于6-10小时）...")
n_before_sleep_filter <- nrow(plot_data_full)
n_sleep_outlier_low <- sum(plot_data_full$sleep_hours_per_day < 6, na.rm = TRUE)
n_sleep_outlier_high <- sum(plot_data_full$sleep_hours_per_day > 10, na.rm = TRUE)
n_sleep_outlier_total <- n_sleep_outlier_low + n_sleep_outlier_high

print(paste("睡眠时间 < 6小时的数量:", n_sleep_outlier_low, "(", round(n_sleep_outlier_low/n_before_sleep_filter * 100, 2), "%)"))
...
# 剔除睡眠时间异常值（保留6-10小时范围内的值）
plot_data_full <- plot_data_full %>%
  filter(sleep_hours_per_day >= 6 & sleep_hours_per_day <= 10)

n_after_sleep_filter <- nrow(plot_data_full)
n_excluded_sleep <- n_before_sleep_filter - n_after_sleep_filter
print(paste("剔除睡眠异常值后样本数:", n_after_sleep_filter))
print(paste("剔除睡眠异常值样本数:", n_excluded_sleep, "(", round(n_excluded_sleep/n_before_sleep_filter * 100, 2), "%)"))

print(paste("原始数据样本数（剔除缺失值和异常值后）:", nrow(plot_data_full)))
...
# 方法1: 使用GAM平滑
smooth_plot_gam <- ggplot(plot_data, aes(x = sleep_hours_per_day, y = residuals)) +
  ...
  # 限制X轴范围到6-10小时（数据已过滤到此范围）
  xlim(6, 10)

# 方法2: 使用LOESS平滑
smooth_plot_loess <- ggplot(plot_data, aes(x = sleep_hours_per_day, y = residuals)) +
  geom_point(alpha = 0.4, size = 0.8, color = "steelblue") +
...
# 方法3: 使用样条平滑
smooth_plot_spline <- ggplot(plot_data, aes(x = sleep_hours_per_day, y = residuals)) +
  geom_point(alpha = 0.4, size = 0.8, color = "steelblue") +
...
# 保存组合图形
ggsave(
  filename = path_result("sleep_cognitive_smooth_plots.png"),
  plot = combined_plot,
  width = 10, 
  height = 15, 
  dpi = 300
...
# 单独保存每个图形
ggsave(
  filename = path_result("sleep_cognitive_gam.png"),
...
ggsave(
  filename = path_result("sleep_cognitive_loess.png"),
...
ggsave(
  filename = path_result("sleep_cognitive_spline.png"),
...
# 计算基本统计信息
sleep_stats <- summary(plot_data$sleep_hours_per_day)
...
# 计算相关性
correlation <- cor(plot_data$sleep_hours_per_day, plot_data$residuals, use = "complete.obs")
print(paste("睡眠总时长与认知变化残差的相关系数:", round(correlation, 4)))

# 保存统计信息
stats_summary <- data.frame(
  Variable = c("sleep_hours_per_day", "cognitive_change_residuals"),
  Mean = c(mean(plot_data$sleep_hours_per_day, na.rm = TRUE), 
           mean(plot_data$residuals, na.rm = TRUE)),
  SD = c(sd(plot_data$sleep_hours_per_day, na.rm = TRUE), 
         sd(plot_data$residuals, na.rm = TRUE)),
  Min = c(min(plot_data$sleep_hours_per_day, na.rm = TRUE), 
          min(plot_data$residuals, na.rm = TRUE)),
  Max = c(max(plot_data$sleep_hours_per_day, na.rm = TRUE), 
...
write.csv(stats_summary, 
          file = path_result("sleep_cognitive_smooth_stats.csv"), 
          row.names = FALSE,
          fileEncoding = "UTF-8")

# 同时保存一个带中文标签的版本（如果系统支持）
stats_summary_chinese <- data.frame(
  Variable = c("睡眠总时长_小时每天", "认知变化残差"),
  Mean = c(mean(plot_data$sleep_hours_per_day, na.rm = TRUE), 
           mean(plot_data$residuals, na.rm = TRUE)),
  SD = c(sd(plot_data$sleep_hours_per_day, na.rm = TRUE), 
         sd(plot_data$residuals, na.rm = TRUE)),
...
write.csv(stats_summary_chinese, 
          file = path_result("sleep_cognitive_smooth_stats_chinese.csv"), 
          row.names = FALSE,
          fileEncoding = "UTF-8")

print("===== 完成 =====")
print("图形已保存到result文件夹中:")
print("- sleep_cognitive_smooth_plots.png (组合图)")
print("- sleep_cognitive_gam.png (GAM平滑)")
print("- sleep_cognitive_loess.png (LOESS平滑)")
print("- sleep_cognitive_spline.png (样条平滑)")
print("统计信息已保存到:")
print("- sleep_cognitive_smooth_stats.csv (英文变量名)")
print("- sleep_cognitive_smooth_stats_chinese.csv (中文变量名)")
